  ...
  const handleSearch = async (searchTerm: string) => {
    try {
      const response = await fetch(
        `https://exercisedb.p.rapidapi.com/exercises?search=${searchTerm}`,
        {
          headers: {
            'X-RapidAPI-Key':
              '4beaf48415mshbf2ce385781a119p15b69ejsnfd3c06c001ec',
    ...
    <div className="container mx-auto">
      ...
      <ul className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {currentResults.map((result) => (
          <li className="flex flex-col items-center rounded-md border p-4 shadow-sm transition duration-200 hover:shadow-lg">
