...
const Index = () => {
  ...
  const getPopularRoutines = () => {
    axios
      .get(`http://localhost:4000/v1/routine/getPopularRoutines`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const onAddExercise = async (routine: IRoutine) => {
    console.log(routine)
    axios
      .post(`http://localhost:4000/v1/routine/add`, routine, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const onDelete = async (id: number) => {
    axios
      .get(`http://localhost:4000/v1/routine/${id}/delete`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      {auth ? (
        <div className="space-y-3 p-3">
          ...
          <div className="border-b border-black pb-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {routines
                .filter((routine) => routine.isFavorite)
